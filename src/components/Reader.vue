  ...
  <div
    ...
    <div class="not-prose border-b pb-4 mb-4 flex flex-col gap-2">
      ...
      <h1 class="text-3xl md:text-4xl lg:text-5xl">
        <a :href="item.url" class="hover:text-slate-900 dark:hover:text-white hover:underline">{{ item.title }}</a>
      </h1>
      <div v-if="item.date_published" class="text-sm">
        <RelativeDate :value="item.date_published" class="font-bold text-slate-500/75" />
      </div>
      <div v-if="item.external_url" class="text-sm">
